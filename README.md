...
- 应用内更新：正式打包版会基于 GitHub Releases 后台检查新版本，发现后自动弹出“应用设置 / 关于”，由用户手动确认下载与安装
- 主要目标平台：Windows x64
...
2. 发现新版本后，会自动弹出“应用设置 / 关于”，展示版本号、发布日期和更新日志。
3. 点击“下载更新”后开始后台下载；关闭弹窗不会中断下载。
4. 下载完成后，可在“应用设置 / 关于”里点击“立即安装”。应用会先 flush 当前未保存内容，再重启进入安装流程。

注意：

- 开发模式不会触发在线更新。
- Windows 是当前优先验证的平台。
- macOS 若仍未签名 / 未公证，应用内不会执行自动替换安装；发现更新后会下载 DMG 并打开，安装确认仍由用户手动完成。
